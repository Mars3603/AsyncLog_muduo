...
class AsyncLogging : boost::noncopyable
{
 ...
 private:
  ...
  typedef muduo::FixedBuffer<muduo::kLargeBuffer> Buffer;  // 开辟了一个固定大小的缓冲区，kLargeBuffer 4M
  typedef std::vector<std::unique_ptr<Buffer>> BufferVector; //使用智能指针来管理buffer
  // typedef boost::ptr_deque<Buffer> BufferList;
  typedef BufferVector::value_type BufferPtr;// 可理解为Buffer的智能指针，能管理Buffer的生存期，
